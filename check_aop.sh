...
log_info "Starting fetch of origin TLS client authentication details from Cloudflare."

response=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$ZONEID/origin_tls_client_auth" \
-H "Authorization: Bearer $BEARER_TOKEN" \
-H "Content-Type: application/json")

if [ $? -eq 0 ]; then
    log_info "Successfully fetched origin TLS client authentication details from Cloudflare API."
    echo "$response" | jq . | tee -a $LOGFILE
    if [[ $(echo "$response" | jq -r '.success') == "true" ]]; then
        log_info "Origin TLS client authentication details fetched successfully."
    else
        log_error "Failed to fetch origin TLS client authentication details. Response: $(echo "$response" | jq -r '.errors[]')"
        exit 1
    fi
else
